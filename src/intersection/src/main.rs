...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lit = LitSubset::from_timespan(&TimeSpan::new_cent(-10, 0), db.authors(), &db.snapshot());
    ...
    let intersection: Vec<_> = db
        .intersect_sources(authors, lit)
        .iter()
        .map(|l| lemma_id_to_string(&db, *l))
        .collect();
